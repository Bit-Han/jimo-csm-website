...
// 			success: true,
// 			message:
// 				"A pending invite already exists for this email. Check Supabase to resend it.",
// 		};
// 	}

// 	// Send via Supabase — their email template handles the branded email
// 	const adminSupabase = createAdminClient();
...
// 	// Record in admin_invitations for audit trail
// 	const expiresAt = new Date();
// 	expiresAt.setDate(expiresAt.getDate() + 7);
...
// 	revalidatePath("/admin/users-roles", "layout");

// 	return {
// 		success: true,
// 		message: `Invite sent to ${data.email} via Supabase email.`,
// 	};
// }
...
// 		return { success: true, message: "User deactivated and sessions revoked." };
// 	} catch (error) {
// 		const message =
// 			error instanceof Error ? error.message : "Unexpected error.";
// 		console.error("[deactivateUser]", message);
...
// 		const callerRole = user.app_metadata?.adminRole as AdminRole | undefined;
// 		if (callerRole !== "super-admin") {
// 			return { success: false, message: "Only Super Admins can change roles." };
// 		}

// 		if (user.id === id && role !== "super-admin") {
// 			return {
// 				success: false,
...
// 		// Update Supabase app_metadata so the JWT reflects the new role
...
// 		return { success: true, message: `Role updated to ${label}.` };
// 	} catch (error) {
// 		const message =
// 			error instanceof Error ? error.message : "Unexpected error.";
// 		console.error("[changeUserRole]", message);
// 		return { success: false, message };
// 	}
// }
...
// 			.set({ status: "revoked" })
// 			.where(eq(adminInvitations.id, inviteId));
...
// 		const header = ["Full Name", "Email", "Role", "Status", "Created At"].join(
// 			",",
// 		);
...
// 		return { success: true, csv: [header, ...csvRows].join("\n"), message: "" };
// 	} catch (error) {
// 		const message =
