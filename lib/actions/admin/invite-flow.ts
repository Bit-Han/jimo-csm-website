...
/**
 * Called from AcceptInvitePage (a Server Component) on every render of that
 * page. By this point /admin/auth/confirm has already exchanged the invite's
 * token_hash for a real session — this function's only job is to check that
 * session belongs to someone with a genuine, pending, unexpired invite, and
 * hasn't already completed signup. This is a UX gate (clear error instead of
 * a broken form); acceptInviteAction re-checks all of this independently at
 * submit time and is the actual security boundary.
 */
export async function resolvePendingInvite(): Promise<VerifyInviteResult> {
	const supabase = await createClient();
