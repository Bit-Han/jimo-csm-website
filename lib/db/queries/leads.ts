...
// 		crmConnected,
// 		crmSyncNote: crmConnected ? "Brevo connected" : "Add BREVO_API_KEY",
// 		totalSynced: 0,
// 		totalLeads: 0,
// 	};

// 	try {
// 		// ✅ Combines 4 queries into 1 raw transaction execution pass!
...
// 	} catch (error) {
// 		console.error(
// 			"[leads:getLeadSummaryStats] Optimised stats query failed:",
// 			error,
// 		);
// 		return defaultStats;
// 	}
...
// // ✅ FIX: Fetches exactly ONE row for prev and ONE row for next using timestamps
// // ─── Prev / next navigation ────────────────────────────────────────────────
// export async function getAdjacentLeadIds(
// 	id: string,
// ): Promise<{ prevId: string | null; nextId: string | null }> {
// 	try {
// 		// Fetch ONLY the required timestamp directly from the table to avoid mapper type mismatches
// 		const currentLeadResult = await withQueryTimeout(
// 			db.select({ createdAt: leads.createdAt }).from(leads).where(eq(leads.id, id)).limit(1),
// 			DB_TIMEOUT_MS,
// 		);
...
// 		// Fetch adjacent rows by comparing the true Date objects
...
// ): Promise<{ position: number; total: number }> {
// 	try {
// 		// Fetch the timestamp directly to prevent structural type mismatches from mappers
// 		const currentLeadResult = await withQueryTimeout(
// 			db.select({ createdAt: leads.createdAt }).from(leads).where(eq(leads.id, id)).limit(1),
// 			DB_TIMEOUT_MS,
// 		);
