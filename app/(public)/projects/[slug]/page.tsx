...
// // Force dynamic during development — switch to revalidate = 60 in production
...
import { MediaCarousel } from "@/components/public/projects/MediaCarousel";
import { getProjectDetailBySlug } from "@/lib/db/queries/projects";
// 1. Import the dynamic site settings query function
import { getPublicSiteSettings } from "@/lib/db/queries/site-settings";

// Force dynamic updates with revalidation rules
export const revalidate = 60;
...
export default async function ProjectDetailPage({ params }: ProjectPageProps) {
	...
	// 2. Fetch the data concurrently using Promise.all to optimize performance
	const [project, siteSettings] = await Promise.all([
		getProjectDetailBySlug(slug),
		getPublicSiteSettings(),
	]);
	...
	return (
		<>
			...
			<MobileBottomDock
				// 4. Pass the database-driven settings here
				whatsappHref={siteSettings.whatsappHref}
				brochureHref={`/brochures/${project.slug}`}
				closingContent={
					<CtaBanner
