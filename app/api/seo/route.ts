// app/api/seo/route.ts
import { NextRequest } from "next/server";
import { getAuthUser } from "@/lib/auth/server";
import { can } from "@/lib/utils/permissions";
import {
	ok,
	unauthorized,
	forbidden,
	serverError,
} from "@/lib/utils/api-response";
import {
	getSeoSettings,
	upsertSeoSettings,
	runSeoAudit,
} from "@/lib/services/seo.service";

export async function GET(_req: NextRequest) {
	try {
		const auth = await getAuthUser();
		if (!auth) return unauthorized();
		if (!can(auth.profile.role, "view_seo")) return forbidden();
		const [settings, audit] = await Promise.all([
			getSeoSettings(),
			runSeoAudit(),
		]);
		return ok({ settings, ...audit });
	} catch (err) {
		return serverError("Failed to load SEO data.", err);
	}
}

export async function PUT(req: NextRequest) {
	try {
		const auth = await getAuthUser();
		if (!auth) return unauthorized();
		if (!can(auth.profile.role, "edit_seo")) return forbidden();
		const body = await req.json();
		const updated = await upsertSeoSettings(body);
		return ok(updated);
	} catch (err) {
		return serverError("Failed to update SEO settings.", err);
	}
}
