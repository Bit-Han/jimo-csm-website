...
export default async function AdminArticleEditPage({
	...
	const state: ArticleEditorState = {
		...
		// insight.content is already guaranteed a valid Tiptap doc by
		// mapInsightRowToDetail — no array/.length check needed here.
		// This defensive re-check is cheap (runs once per page load, not
		// per keystroke) and just guards against any future caller of
		// mapInsightRowToDetail that might not go through the same guard.
		content: isValidTiptapDoc(insight.content)
			? insight.content
			: EMPTY_TIPTAP_DOC,
