...
import { inputCls, selectCls } from "@/components/admin/ui/EditorField";
import {
	ALLOWED_MAPPINGS_BY_FIELD_TYPE,
	CRM_MAPPING_OPTIONS,
} from "@/lib/constants/crm-mapping";
import type {
	FormBuilderField,
	FormFieldOption,
} from "@/lib/types/admin/form-builder";
...
export function FieldSettingsPanel({
	...
	// Self-heals fields saved before mapping was locked, or a field whose
	// type only ever has one valid mapping — keeps that mapping correct
	// without giving the admin a control that could break it.
	useEffect(() => {
		if (!field) return;
		const allowed = ALLOWED_MAPPINGS_BY_FIELD_TYPE[field.type] ?? [];
		if (allowed.length === 1 && field.crmMapping !== allowed[0]) {
			onUpdate(field.id, { crmMapping: allowed[0] });
		}
	}, [field?.id, field?.type, field?.crmMapping, onUpdate, field]);
	...
	const hasOptions = field.type === "dropdown" || field.type === "radio";
	const allowedValues = ALLOWED_MAPPINGS_BY_FIELD_TYPE[field.type] ?? [];
	const allowedOptions = CRM_MAPPING_OPTIONS.filter((o) =>
		allowedValues.includes(o.value),
	);
	const isLocked = allowedOptions.length <= 1;
			...
			<div className="mt-4 flex-1 space-y-4">
				...
				<div>
					...
					<select
						value={field.width}
						onChange={(e) =>
							update({ width: e.target.value as FormBuilderField["width"] })
						}
						className={selectCls}
					>
						<option value="full">Full width</option>
				...
				{/* CRM Mapping — locked to a fixed set, per field type. */}
				{field.type !== "consent" ? (
					<div>
						<label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-stone-500">
							CRM Mapping
						...
						{isLocked ? (
							<div className="flex items-center justify-between rounded-lg border border-stone-200 bg-stone-50 px-3 py-2.5 text-sm text-stone-600">
								<span>{allowedOptions[0]?.label ?? "Not mapped"}</span>
						...
						<p className="mt-1 text-[11px] text-stone-400">
							{isLocked
								? "This field type only ever feeds one CRM column, so it's fixed."
								: "Restricted to the CRM columns that make sense for this field type."}
						</p>
					</div>
				) : null}
