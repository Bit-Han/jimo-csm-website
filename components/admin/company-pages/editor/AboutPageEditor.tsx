...
function SaveRow({
	...
	return (
		<div className="mt-4 flex items-center justify-end gap-3 border-t border-stone-100 pt-4">
			{result ? (
				<span
					className={`text-xs font-medium ${result.ok ? "text-emerald-600" : "text-red-500"}`}
				>
					{result.text}
				</span>
			) : null}
...
export function AboutPageEditor({
	...
	// Photos queued for deletion once the user confirms a replace/remove —
	// nothing actually hits Supabase Storage until Save succeeds.
	...
	function queueDeletion(url?: string) {
		if (!url) return;
	...
	function handleConfirmRemoveMember() {
		...
		const member = teamMembers[removeConfirmIndex];
		if (member?.photo.src) queueDeletion(member.photo.src);
		setTeamMembers((prev) => prev.filter((_, j) => j !== removeConfirmIndex));
	...
	return (
		<div className="space-y-4">
			...
			>
				...
				<SaveRow
					onSave={async () => {
						const result = await saveTeamMembers(
							teamMembers,
							pendingImageDeletions,
						);
						if (result.success) setPendingImageDeletions([]);
						return result;
					}}
				/>
			</Accordion>
			...
			<ConfirmDialog
				...
				description="Their profile photo will be permanently deleted from storage when you save. This can't be undone."
				confirmLabel="Remove"
				variant="danger"
