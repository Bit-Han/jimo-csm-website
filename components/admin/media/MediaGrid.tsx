...
import { deleteMediaAsset } from "@/lib/actions/admin/media";
import { cn } from "@/lib/utils/helpers";
import type { MediaAsset, MediaSortMode } from "@/lib/types/admin/media";
...
export function MediaGrid({
	...
	const [sort, setSort] = useState<MediaSortMode>("newest");
	...
	const filtered = assets.filter((a) => {
		...
		const matchType =
			fileTypeFilter === "all" || a.resourceType === fileTypeFilter;
		return matchSearch && matchType;
	});
	...
	async function handleUpload(file: File) {
		...
		fd.append("file", file);
		fd.append("folder", "jimo-property/project-renders");
		...
		try {
			...
			if (data.success && onUploadComplete) {
				onUploadComplete({
					id: data.asset.id,
					cloudinaryPublicId: data.asset.publicId,
					url: data.asset.secureUrl,
					name: data.asset.altText,
					projectName: null,
					tagLabel: data.asset.folder,
					tagColorClass: "bg-teal-50 text-teal-700",
					resourceType: data.asset.resourceType,
					format: data.asset.format,
					sizeLabel: data.asset.sizeLabel,
				});
			}
		} finally {
			setUploading(false);
	...
	async function handleDelete(id: string) {
		const confirmed = window.confirm(
			"Delete this asset? This removes it from Cloudinary too.",
		);
		if (!confirmed) return;
		setDeletingId(id);
		await deleteMediaAsset(id);
		...
		<div className="flex flex-col gap-4 rounded-2xl border border-stone-200 bg-white p-5">
			...
			{loading ? (
				...
				<div className="flex flex-col items-center gap-3 py-16 text-center">
					<Upload className="h-8 w-8 text-stone-300" />
					<p className="text-sm text-stone-400">No media files yet.</p>
					<button
						type="button"
				...
				<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
					...
					{uploading ? (
						<div className="flex aspect-[4/3] items-center justify-center rounded-2xl border-2 border-dashed border-red-300 bg-red-50">
