...
// //@components/admin/projects/projectsExplorer.tsx
// "use client";
...
import { deleteProject, unpublishProject } from "@/lib/actions/admin/project";
import { cn } from "@/lib/utils/helpers";
import type {
	AdminDisplayStatus,
	AdminProjectListRow,
...
export function ProjectsExplorer({ projects: initialProjects }: ProjectsExplorerProps) {
	...
	const [confirmDelete, setConfirmDelete] = useState<AdminProjectListRow | null>(null);
	...
	const filtered = useMemo(() => {
		return projects.filter((p) => {
			...
			const matchesStatus = filters.status === "all" || p.adminStatus === filters.status;
	...
	function handleUnpublish(project: AdminProjectListRow) {
		setBusyId(project.id);
		startTransition(async () => {
			const result = await unpublishProject(project.slug);
			if (result.success) {
				setProjects((prev) =>
					prev.map((p) => (p.id === project.id ? { ...p, publishStatus: "draft" } : p)),
				);
			} else {
	...
	function handleConfirmDelete() {
		...
		startTransition(async () => {
			const result = await deleteProject(confirmDelete.slug);
			if (result.success) {
	...
	return (
		<div className="space-y-4">
			{/* ── Filter bar — unchanged from before ── */}
			<div className="flex flex-wrap items-center gap-3">
				<div className="relative">
					<Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400" />
					<input
				...
				<select value={filters.location} onChange={(e) => set("location", e.target.value)} className={selectCn}>
					<option value="all">Location — All</option>
					{uniqueLocations.map((loc) => (
						<option key={loc} value={loc}>
							{loc}
			...
			<div className="overflow-hidden rounded-2xl border border-stone-200 bg-white">
				<div className="overflow-x-auto">
					<table className="w-full min-w-[820px] text-left text-sm">
						<thead>
							<tr className="border-b border-stone-100 bg-stone-50/60">
								{["Project", "Location", "Status", "Starting Price", "Leads", "Last Updated", "Actions"].map(
									(h) => (
										<th
											key={h}
											className="px-5 py-3.5 text-xs font-semibold uppercase tracking-wide text-stone-500"
							...
							{filtered.length === 0 ? (
								...
								filtered.map((project) => (
									<tr
										...
										className="border-b border-stone-100 transition-colors last:border-none hover:bg-stone-50"
									>
										<td className="px-5 py-4 font-semibold text-ink-950">{project.name}</td>
										<td className="px-5 py-4 text-stone-600">{project.location}</td>
										<td className="px-5 py-4">
											<AdminBadge variant={STATUS_BADGE[project.adminStatus]} />
										</td>
										<td className="px-5 py-4 text-stone-600">{project.startingPrice}</td>
										<td className="px-5 py-4">
											<div className="flex items-center gap-1.5">
												<span className="font-medium text-ink-950">
												...
												{project.leads > 0 && (
													<span
														className={cn(
															"flex items-center gap-0.5 text-xs font-medium",
															project.leadChangePercent >= 0 ? "text-emerald-600" : "text-red-500",
														)}
													>
														{project.leadChangePercent >= 0 ? (
															<TrendingUp className="h-3 w-3" />
										...
										<td className="px-5 py-4">
											<p className="text-stone-600">{project.lastUpdatedDate}</p>
											<p className="mt-0.5 text-xs text-stone-400">{project.lastUpdatedBy}</p>
										</td>
										<td className="px-5 py-4">
											<ProjectRowMenu
												project={project}
												busy={busyId === project.id && isPending}
												onUnpublish={() => handleUnpublish(project)}
			...
			<ConfirmDialog
				...
				description="This permanently deletes the project, its cover image, and every gallery image from Cloudinary. This can't be undone."
