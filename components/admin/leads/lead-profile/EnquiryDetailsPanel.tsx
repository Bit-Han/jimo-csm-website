...
const DT = "text-xs font-semibold uppercase tracking-wide text-stone-500";
const DD = "mt-1 text-sm font-semibold text-ink-950";
...
export function EnquiryDetailsPanel({ lead }: { lead: LeadDetail }) {
	return (
		<div className="rounded-2xl border border-stone-200 bg-white p-6">
			...
			<dl className="mt-4 grid gap-4 sm:grid-cols-2">
				<div>
					<dt className={DT}>Project Interest</dt>
					<dd className={DD}>{lead.projectPage}</dd>
				</div>
				<div>
					<dt className={DT}>Budget Range</dt>
					<dd className={DD}>{lead.budget}</dd>
				</div>
				{lead.enquiryType ? (
					<div>
						<dt className={DT}>Enquiry Type</dt>
						<dd className={`${DD} capitalize`}>
			...
			{lead.message ? (
				<div className="mt-4 border-t border-stone-100 pt-4">
					<dt className={`${DT} mb-1`}>Message</dt>
					<p className="text-sm leading-relaxed text-stone-600">
						{lead.message}
					</p>
				</div>
