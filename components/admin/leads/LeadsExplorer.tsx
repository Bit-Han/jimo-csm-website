// //@/components/admin/leads/LeadsExplorer.tsx
// "use client";
...
// ─── Row menu (three dots → Delete, View) ──────────────────────────────────
// Rendered via portal into document.body, same reasoning as InsightsExplorer's
// RowMenu: a menu positioned inside the table's overflow-x-auto wrapper can
// get silently clipped by the vertical overflow CSS forces alongside
// horizontal auto-scroll. Real screen coordinates sidestep that, and it
// works identically for the mobile card layout.
						...
						<>
							...
							>
								...
								<button
									...
									className="flex w-full px-3 py-2 text-left text-xs font-medium text-ink-950 hover:bg-stone-50"
								>
									View
								</button>
							</div>
...
export function LeadsExplorer({
	...
	const [confirmDelete, setConfirmDelete] = useState<LeadListRow | null>(null);
	...
	function handleConfirmDelete() {
		...
		startDeleteTransition(async () => {
			const result = await deleteLead(confirmDelete.id);
			if (result.success) {
				const next = new Set(selectedIds);
	...
	return (
		<div className="space-y-4">
								...
								<tbody>
									{rows.map((lead) => (
										<LeadTableRow
											...
											selected={selectedIds.has(lead.id)}
											onToggle={() => toggleRow(lead.id)}
											busy={busyId === lead.id && isDeleting}
			...
			<ConfirmDialog
				...
				title={confirmDelete ? `Delete lead for "${confirmDelete.name}"?` : ""}
				description="This permanently removes this lead and its notes from the database. This can't be undone."
...
function LeadTableRow({
	...
	return (
		...
		>
			...
			<td className="py-4 pr-4 text-right">
				<LeadRowMenu lead={lead} busy={busy} onDelete={onDelete} />
			</td>
		</tr>
	);
}
