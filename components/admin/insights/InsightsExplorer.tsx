...
const SEO_STATUS_CONFIG: Record<
	ArticleSeoStatus,
		...
		<div className="relative">
			...
			{open ? (
				<>
					...
					/>
					<div className="absolute right-0 top-9 z-20 w-44 overflow-hidden rounded-xl border border-stone-200 bg-white py-1 shadow-lg">
...
export function InsightsExplorer({
	...
	const [confirmDelete, setConfirmDelete] =
		useState<AdminArticleListRow | null>(null);
	...
	function handleUnpublish(article: AdminArticleListRow) {
		...
		startTransition(async () => {
			...
			if (result.success) {
				setArticles((prev) =>
					prev.map((a) =>
						a.id === article.id ? { ...a, publishStatus: "draft" } : a,
					),
				);
			}
			setBusyId(null);
