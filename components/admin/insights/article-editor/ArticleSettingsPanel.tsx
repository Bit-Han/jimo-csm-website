// //@components/admin/insights/article-editor/ArticleSettingsPanel.tsx
...
import { addInsightCategory } from "@/lib/actions/admin/insight-categories";
import { featuredProjects } from "@/lib/data/projects";
import type {
	ArticleEditorState,
	AuthorOption,
...
export function ArticleSettingsPanel({
	...
	function handleAddCategory() {
		...
		startTransition(async () => {
			const result = await addInsightCategory(label);
			if (result.success && result.category) {
				setCategories((prev) =>
					prev.some((c) => c.value === result.category!.value)
						? prev
						: [...prev, result.category!],
				);
				onChange("category", result.category!.value as InsightCategory);
				onChange("categoryLabel", result.category!.label);
				setNewCategoryLabel("");
		...
		<div className="flex flex-col gap-5 rounded-2xl border border-stone-200 bg-white p-6">
			...
			<div>
				...
				<AuthorPicker
					...
					onChange={(author) => {
						onChange("authorId", author.id);
						onChange("authorName", author.fullName);
						onChange("authorAvatarUrl", author.avatarUrl ?? "");
					}}
				/>
			</div>
			...
			<div>
				...
				<div className="flex gap-2">
					<select
						value={state.category}
						onChange={(e) => {
							const found = categories.find((o) => o.value === e.target.value);
							onChange("category", e.target.value as InsightCategory);
							onChange("categoryLabel", found?.label ?? "");
						}}
					...
					<button
						...
						className="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-lg border border-stone-200 text-stone-500 hover:bg-stone-50"
					>
				...
				{addingCategory ? (
					<div className="mt-2 flex gap-2">
						...
						<button
							...
							className="shrink-0 rounded-lg bg-red-600 px-3 text-xs font-semibold text-white hover:bg-red-700 disabled:opacity-50"
						>
							Add
						</button>
					</div>
